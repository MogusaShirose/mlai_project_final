...
while True:
    ...
    if max_confidence.item() >= confidence_threshold:
        ...
        # Define bounding box coordinates (approximate center of frame)
        box_top_left = (int(width*0.3), int(height*0.3))
        box_bottom_right = (int(width*0.7), int(height*0.7))
        
        # Draw bounding box around the detected fruit
        cv2.rectangle(frame, box_top_left, box_bottom_right, (0, 255, 0), 2)
        
        # Put text above the bounding box
        text_location = (box_top_left[0], box_top_left[1] - 10)
        cv2.putText(frame, display_text, text_location, cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
    else:
        predicted_class = 'Unknown'
        display_text = f'Prediction: {predicted_class}'
        
        # Put text in the frame without bounding box
